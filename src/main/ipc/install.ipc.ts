...
export function registerInstallHandlers(win: BrowserWindow) {
  ipcMain.handle(IPC.INSTALL_START, async (_event, serverId: string, downloadUrl?: string, sudoPassword?: string, skipUpdates?: boolean, artifactName?: string) => {
    // Run async — progress events are pushed via win.webContents.send
    runInstall(serverId, win, downloadUrl, sudoPassword, skipUpdates, artifactName).catch((err) => {
